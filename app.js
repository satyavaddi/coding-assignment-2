...
app.get("/user/tweets/feed/", authentication, async (request, response) => {
  ...
  const followingPeople = await getFollowingPeopleIdsOfUser(username);
...
app.get(
  ...
  async (request, response) => {
    ...
    const likedUser = await database.all(getLikesQuery);
...
app.get(
  ...
  async (request, response) => {
    ...
    const repliedUser = await database.all(getRepliedQuery);
    response.send(repliedUser);
  }
);
...
app.get("/user/tweets/", authentication, async (request, response) => {
  const { userId } = request;
  getTweetsQuery = `SELECT tweet,
    COUNT(DISTINCT like_id) as likes,
    COUNT(DISTINCT reply_id)as replies,
    date_time as dateTime
    FROM user LEFT JOIN reply ON tweet.tweet_id = reply.tweet_id
...
app.delete("/tweets/:tweetId/", authentication, async (request, response) => {
  const { tweetId } = request.params;
  const { userId } = request;
  const getTheTweetQuery = `SELECT * FROM user WHERE user_id = '${userId}' AND tweet_id = '${tweetId};'`;
